...
void Sprockets::move() {
    switch (state) {
        ...
        case State::INTAKE:
            ...
            intake_motor.move(-SPROCKET_VOLTAGE);
            if (low_motor.get_torque() < 0.5) low_motor.move(SPROCKET_VOLTAGE); else low_motor.move(SPROCKET_VOLTAGE / 3);
            if (middle_motor.get_torque() < 0.5) middle_motor.move(-SPROCKET_VOLTAGE / 3); else middle_motor.move(-SPROCKET_VOLTAGE / 4);
            high_motor.brake();
            break;
        case State::NONE:
        default:
